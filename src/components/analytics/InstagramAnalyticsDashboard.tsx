...
import {
  InstagramUserAnalyticsResponse,
  InstagramPostAnalyticsResponse,
  InstagramAnalyticsError,
} from "@/lib/types/instagram-analytics";
import {
  handleApiError,
  isAuthError,
  isRateLimitError,
...
export default function InstagramAnalyticsDashboard({
  ...
  const [userAnalytics, setUserAnalytics] =
    useState<InstagramUserAnalyticsResponse | null>(null);
  const [postAnalytics, setPostAnalytics] =
  ...
  const getPostAnalyticsCard = (post: any) => (
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button
            ...
            disabled={loading}
            className="mt-6 px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 disabled:opacity-50"
          >
        ...
        <>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-6">
              ...
              {getMetricCard(
                "Comments",
                userAnalytics.data.total_comments,
                <MessageCircle className="w-6 h-6 text-blue-600" />,
                "bg-blue-100"
              )}
              {getMetricCard(
                "Shares",
                userAnalytics.data.total_shares,
                <Share className="w-6 h-6 text-green-600" />,
                "bg-green-100"
              )}
              {getMetricCard(
                "Saves",
                userAnalytics.data.total_saves,
                <Bookmark className="w-6 h-6 text-indigo-600" />,
                "bg-indigo-100"
              )}
              {getMetricCard(
                "Profile Visits",
                userAnalytics.data.total_profile_visits,
                <MousePointer className="w-6 h-6 text-pink-600" />,
                "bg-pink-100"
              )}
              {getMetricCard(
                "Website Clicks",
                userAnalytics.data.total_website_clicks,
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="mt-2 text-sm text-gray-600">
              Total posts analyzed: {userAnalytics.data.total_posts}
            </div>
          </div>
        </>
      )}
      ...
      {postAnalytics && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {postAnalytics.data.map(getPostAnalyticsCard)}
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">
              Connect your Instagram account and load analytics to see detailed
              performance metrics.
            </p>
            <button
              onClick={fetchUserAnalytics}
